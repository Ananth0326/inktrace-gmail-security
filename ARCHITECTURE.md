...
![InkTrace Architecture Diagram](./architecture.png)

*(Generated from Graphviz `architecture` snippet)*
...
Unlike traditional scanners, InkTrace uses a tiered approach to classify emails:
-   **Layer 1 (Linguistic)**: Matches against known phishing keywords and malicious URL patterns via the Rule Engine (Regex Checks).
-   **Layer 2 (Semantic)**: Retrieves the top-K most similar examples from the user's past history using Cosine Similarity (FastEmbed).
-   **Layer 3 (Cognitive)**: Uses a structured prompt to ask the LLM (Llama 3 via Groq API): *"Based on the rules found and these similar historical examples, is this email a threat?"*
...
-   **Local Storage**: No third-party tracking or remote database syncing. Your data stays securely on your machine.
